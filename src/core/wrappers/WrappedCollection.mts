...
> implements Collection<T> {
  ...
  all(): Filtered<T> {
    ...
    return {
      ...
      async get() {
        ...
        return record === null ? null : self._unwrapAll(record);
      },
      async *values() {
        for await (const record of baseFilter.values()) {
          yield await self._unwrapAll(record);
        }
      },
      async remove() {
      ...
      async update(delta: Partial<T>, options?: UpdateOptions) {
        const converted = await self._wrapAll(delta);
  ...
  where<K extends string & keyof T>(attribute: K, value: T[K]): Filtered<T> {
    ...
    const self = this;
    const baseFilter = this._baseCollection.where(attribute, value as unknown as Inner[K]);
    const extra = makeKeyValue(attribute, value);
    return {
      ...baseFilter,
  ...
  /** @internal */ private async _wrapAll(
    ...
    let processed: E;
    if (this._wrapper.preWrap && hasAnyAttribute(record, this._attributes)) {
  ...
  /** @internal */ private async _unwrapAll<K extends keyof T>(
    ...
    let processed: E;
    if (this._wrapper.preUnwrap && hasAnyAttribute(record, this._attributes)) {
      const allAttributes = extra ? { ...extra, ...record } : record;
